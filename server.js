...
client.connect(()=>{console.log("connected to memcache on local host port 11211!")});
...
app.get("/hw7",(req,res,next)=>{
	...
	//{ club:, pos:, max_assists:, player:, avg_assists:}
			connection.query('select * from `assists` where `POS` = ? and `club` = ? ORDER BY A DESC, GS DESC',[position,club], function(err,player){
		//console.log(err)
		//console.log(player)
				if(err)
